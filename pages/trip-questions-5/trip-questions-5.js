...
Page({
  data: {
    ...
    // 问题数据
    question: {
      title: '以下哪些体验会严重影响你的旅行心情? (Ai将优化行程,避免踩雷)',
      options: [
        { id: 1, text: '过度赶路 (连续2天以上跨城/单日超2万步)', icon: '', selected: false },
        { id: 2, text: '时间浪费 (凌晨航班/排队超1小时/用餐时奔波)', icon: '', selected: false },
        { id: 3, text: '体验不佳 (网红扎堆地/商业化过重区域)', icon: '', selected: false },
        { id: 4, text: '重复行程 (相似景点连逛)', icon: '', selected: false },
        { id: 5, text: '不便因素 (大量台阶/夜间交通盲区)', icon: '', selected: false },
        { id: 6, text: '其他 (如"拒绝购物点""不吃连锁店")', icon: '', selected: false }
      ]
    },
  ...
  onLoad(options) {
    console.log('trip-questions-5页面加载，参数:', options)
  ...
  initPageData() {
    // 从缓存中恢复已选择的选项
    const savedOptions = wx.getStorageSync('tripQuestions5SelectedOptions') || []
  ...
  selectOption(e) {
    ...
    wx.setStorageSync('tripQuestions5SelectedOptions', selectedOptionDetails)
  ...
  nextStep() {
    ...
    wx.redirectTo({
      url: '/pages/trip-questions-8/trip-questions-8',
      success: () => {
  ...
  // 页面显示
  onShow() {
    console.log('trip-questions-5页面显示')
  },

  // 页面隐藏
  onHide() {
    console.log('trip-questions-5页面隐藏')
  }
}) 