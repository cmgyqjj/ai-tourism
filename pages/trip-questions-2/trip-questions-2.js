...
Page({
  data: {
    ...
    question: {
      ...
      options: [
        ...
        { id: 3, text: '美食雷达', icon: '🍱', selected: false },
        { id: 4, text: '摄影狂魔', icon: '📷', selected: false },
        { id: 5, text: '松弛舒适', icon: '🌲', selected: false },
        { id: 6, text: '省钱砖家', icon: '🐷', selected: false },
        { id: 7, text: '人文历史', icon: '⏳', selected: false },
        { id: 8, text: '艺术爱好', icon: '🎨', selected: false },
        { id: 9, text: '音乐迷', icon: '🎧', selected: false },
        { id: 10, text: '自驾旅行', icon: '🚗', selected: false },
        { id: 11, text: '奢华享受', icon: '💎', selected: false },
        { id: 12, text: '夜猫子', icon: '🌙', selected: false },
        { id: 13, text: '户外探险', icon: '🏝️', selected: false },
        { id: 14, text: '购物达人', icon: '🛍️', selected: false },
        { id: 15, text: '游乐园', icon: '🎡', selected: false }
      ]
    },
  ...
  onLoad(options) {
    console.log('trip-questions-2页面加载，参数:', options)
  ...
  initPageData() {
    // 从缓存中恢复已选择的选项
    const savedOptions = wx.getStorageSync('tripQuestions2SelectedOptions') || []
  ...
  selectOption(e) {
    ...
    wx.setStorageSync('tripQuestions2SelectedOptions', selectedOptionDetails)
  ...
  nextStep() {
    ...
    console.log('选择的旅行性格标签:', selectedTexts)
    
    // 将行程信息传递给下一个页面
    const nextPageUrl = `/pages/trip-questions-3/trip-questions-3?tripInfo=${encodeURIComponent(JSON.stringify(tripInfo))}`
    
    // 跳转到下一个问题页面
    wx.redirectTo({
      url: nextPageUrl,
      success: () => {
        console.log('跳转到第3个问题页面成功')
      },
      fail: (error) => {
        console.error('跳转到第3个问题页面失败:', error)
        wx.showToast({
          title: '页面跳转失败',
          icon: 'none',
          duration: 2000
  ...
  // 页面显示
  onShow() {
    console.log('trip-questions-2页面显示')
  },

  // 页面隐藏
  onHide() {
    console.log('trip-questions-2页面隐藏')
  }
}) 