...
Page({
  data: {
    ...
    question: {
      title: '住宿你最关注的点 (可多选):',
      options: [
        { id: 1, text: '星级 / 舒适度', icon: '', description: '(五星 / 四星优先)', selected: false },
        { id: 2, text: '地理位置', icon: '', description: '(景区核心区 / 地铁直达 / 商圈...)', selected: false },
        { id: 3, text: '价格性价比', icon: '', description: '(预算有限, 省钱不挑)', selected: false },
        { id: 4, text: '特色体验', icon: '', description: '(树屋 / 海景房 / 古堡民宿...)', selected: false },
        { id: 5, text: '便利设施', icon: '', description: '(含早餐 / 接送服务 / 泳池)', selected: false }
      ]
    },
  ...
  onLoad(options) {
    console.log('trip-questions-6页面加载，参数:', options)
  ...
  initPageData() {
    // 从缓存中恢复已选择的选项
    const savedOptions = wx.getStorageSync('tripQuestions6SelectedOptions') || []
  ...
  selectOption(e) {
    ...
    wx.setStorageSync('tripQuestions6SelectedOptions', selectedOptionDetails)
  ...
  nextStep() {
    ...
    console.log('选择的住宿关注点:', selectedTexts)
    
    // 将行程信息传递给下一个页面
    const nextPageUrl = `/pages/trip-questions-7/trip-questions-7?tripInfo=${encodeURIComponent(JSON.stringify(tripInfo))}`
    
    // 跳转到下一个问题页面
    wx.redirectTo({
      url: nextPageUrl,
      success: () => {
        console.log('跳转到第7个问题页面成功')
      },
      fail: (error) => {
        console.error('跳转到第7个问题页面失败:', error)
        wx.showToast({
          title: '页面跳转失败',
          icon: 'none',
          duration: 2000
  ...
  // 页面显示
  onShow() {
    console.log('trip-questions-6页面显示')
  },

  // 页面隐藏
  onHide() {
    console.log('trip-questions-6页面隐藏')
  }
}) 