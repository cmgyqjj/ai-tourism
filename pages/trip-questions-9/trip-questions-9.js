...
Page({
  data: {
    ...
    question: {
      ...
      options: [
        ...
        { id: 3, text: '性价比 (人均预算可控)', icon: '', selected: false },
        { id: 4, text: '特殊需求 (素食/清真/无麸质/减肥人士)', icon: '', selected: false }
      ]
    },
  ...
  onLoad(options) {
    console.log('trip-questions-9页面加载，参数:', options)
  ...
  initPageData() {
    // 从缓存中恢复已选择的选项
    const savedOptions = wx.getStorageSync('tripQuestions9SelectedOptions') || []
  ...
  selectOption(e) {
    ...
    wx.setStorageSync('tripQuestions9SelectedOptions', selectedOptionDetails)
  ...
  nextStep() {
    ...
    console.log('选择的就餐偏好:', selectedTexts)
    
    // 将行程信息传递给下一个页面
    const nextPageUrl = '/pages/trip-questions-10/trip-questions-10?tripInfo=' + encodeURIComponent(JSON.stringify(tripInfo))
    
    // 跳转到下一个问题页面
    wx.redirectTo({
      url: nextPageUrl,
      success: () => {
        console.log('跳转到第10个问题页面成功')
      },
      fail: (error) => {
        console.error('跳转到第10个问题页面失败:', error)
        wx.showToast({
          title: '页面跳转失败',
          icon: 'none',
          duration: 2000
  ...
  // 页面显示
  onShow() {
    console.log('trip-questions-9页面显示')
  },

  // 页面隐藏
  onHide() {
    console.log('trip-questions-9页面隐藏')
  }
})